...
struct RendererDX11 final : Renderer {
  ...
  inline ImTextureID ImGui_LoadTexture(const char *path) {
    return nullptr;
  }
  inline ImTextureID ImGui_CreateTexture(
      const void *data, int width, int height) {
    return nullptr;
  }
  inline void ImGui_DestroyTexture(ImTextureID texture) {
  }
  inline int ImGui_GetTextureWidth(ImTextureID texture) {
    return 0;
  }
  inline int ImGui_GetTextureHeight(ImTextureID texture) {
    return 0;
  }
...
void RendererDX11::Clear(const ImVec4 &color) {
  m_deviceContext->ClearRenderTargetView(
      m_mainRenderTargetView, (float *)&color.x);
}
...
HRESULT RendererDX11::CreateDeviceD3D(HWND hWnd) {
  ...
  if (D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr,
          ...
          &m_swapChain, &m_device, &featureLevel, &m_deviceContext) != S_OK)
    return E_FAIL;
...
void RendererDX11::CreateRenderTarget() {
  ...
  m_device->CreateRenderTargetView(
      pBackBuffer, &render_target_view_desc, &m_mainRenderTargetView);
...
ImTextureID RendererDX11::CreateTexture(
    const void *data, int width, int height) {
