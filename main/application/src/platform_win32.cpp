...
std::wstring Utf8ToNative(const std::string &str) {
  ...
  MultiByteToWideChar(
      CP_UTF8, 0, str.data(), (int)str.size(), (wchar_t *)result.data(), size);
  return result;
}
#else
...
bool PlatformWin32::ApplicationStart(int argc, char **argv) {
  ...
  m_WindowClass = {sizeof(WNDCLASSEX), CS_CLASSDC, winProc, 0L, 0L,
...
bool PlatformWin32::OpenMainWindow(const char *title, int width, int height) {
  ...
  m_MainWindowHandle = CreateWindow(m_WindowClass.lpszClassName,
      Utf8ToNative(title).c_str(), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
      CW_USEDEFAULT, width < 0 ? CW_USEDEFAULT : width,
      height < 0 ? CW_USEDEFAULT : height, nullptr, nullptr,
